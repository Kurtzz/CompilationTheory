...
class Interpreter(object):
    ...
    @when(AST.InstructionIf)
    def visit(self, node):
        if node.condition.accept(self):
            node.instructionIf.accept(self)
        elif node.instructionElse is not None:
            node.instructionElse.accept(self)

    @when(AST.InstructionWhile)
    def visit(self, node):
